class cpu:
    ...
    def ANC(self):
        return 0
    def SAX(self):
        self.bus.cpuWrite(self.addr_abs, self.a & self.x)
        return 0
    def RLA(self):
        self.ROL()
        self.AND()
    ...
    def __init__(self, bus):
        self.cpuramblock = bytearray(2048)
        self.cpuram = memoryview(self.cpuramblock)
        self.bus = bus
        self.stopped = False
        self.a = 0
        self.x = 0
    ...
    def clock(self):
        if not self.cycles:
            ...
            if self.bus.debugmode:
                print("{}, {}, {}, A:{}, X:{}, Y:{}, P:{}, SP:{}".format(hex(self.pc), hex(self.opcode), self.lookup[self.opcode][0], hex(self.a), hex(self.x), hex(self.y), hex(self.status), hex(self.stkp)))
            self.pc += 1
    ...
    def nmi(self):
        self.tempaddr = 0x0100 + self.stkp
        self.write(self.tempaddr, (self.pc >> 8) & 0xFF)
        self.write(self.tempaddr - 1, self.pc & 0xFF)
        self.flagB = 0
        self.flagU = 1
        self.flagI = 1
        self.status = self.flagC | (self.flagZ << 1) | (self.flagI << 2) | (self.flagD << 3) | (self.flagB << 4) | (self.flagU << 5) | (self.flagV << 6) | (self.flagN << 7)
